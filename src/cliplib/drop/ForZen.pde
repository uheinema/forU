...
public class ForZen implements ParamProvider  {
  ...
  public ForZen (PathDrawer dr) {  
    //his.dr=dr;
    this.dude=new Doodle(new Drawer(dr));
    begin();
  }
...
*/
  ...
  public ForZen execute(String s)//ArrayList<String> passed)
  {
   ...
   // dump();
    ...
    for (String to=t.next(); to!=null; to=t.next()) {
      ...
      if ('$'==to.charAt(0)) {
        ...
        if (scanParam>0) {
          ...
          appendParam(getParam(mytop-np-1));
        } else {
        //  println("executing param -"+np);
          execute(getParam(mytop-np-1));
          dropParams(mytop);
        }
        continue;
      }
    ...
    // we may end in the middle of
    // a patam definition,
    // so what is the frame for the next call?
    //lastFrame=mytop;
    return this;
  }
