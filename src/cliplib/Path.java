...
public class Path extends ArrayList<LongPoint> {
    ...
    public Path cleanPolygon(double distance ) {
        ...
        int cnt = size();
   if(true){
     PApplet.println("cleanp called, ignore");
   return this;
   }
        if (cnt == 0) {
            return new Path();
    ...
    // bunch of after the fact shit, but missing:
    public Path add(double x,double y){
  add(new LongPoint((long)Math.round(x),(int)Math. round(y)));
  return this;
