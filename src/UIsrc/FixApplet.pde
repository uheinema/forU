...
```
*/
// BUG/Workaround: APDE Preview can not load assets from subdirs

// To demonstrate, create a textfile
// sketch/data/I/i.exist
// due to a bug(?) in PApplet/PreviewUtil interaction
...
boolean isReallyPreview(){
 ...
 String dataFolder = 
  ...
  // PApplet.createInputRaw()) to internal
  // 
...
boolean isPreview() {
  ...
  try {
    if (loadStrings( exist )!=null) {
     ...
     UI. flash("Oops, loadStrings() returned null?");
  }
  catch(Exception e) {
    // flash("Preview");
    ...
    try {
      ...
      if (loadStrings( sketchPath("") +"/"+exist)!=null) {
       UI. flash("Preview, found in local storage");
      } else
       UI. flash("Ahbah..never happens?");
    }
    catch(Exception ee)
    {
      UI. toast("No "+exist+" in assets or "+sketchPath("")+"/"+exist+
